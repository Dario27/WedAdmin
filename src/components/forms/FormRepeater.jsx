...
const FormRepeater = () => {
  ...
  return (
    <form className="form-inline" onSubmit={(event) => event.preventDefault()}>
        ...
        {users.map((user, index) => (
          <div key={user.id} className="d-flex mb-2">
            ...
            <div className='input-group-append'>
                {index > 0 && (
                    <Button variant='btn btn-danger icon-btn' size='sm' className="ms-2" onClick={() => deleteUser(index)} >
      ...
      <Button variant='btn btn-info icon-btn' size='sm' className="ms-2 mb-2" onClick={addUserInput} >
